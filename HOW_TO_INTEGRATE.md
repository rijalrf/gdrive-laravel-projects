...
Dokumen ini menjelaskan cara mengintegrasikan dan menggunakan fitur backup database PostgreSQL ke Google Drive pada proyek ini.

## Prasyarat & Kebutuhan Sistem
1. Proyek ini berjalan menggunakan Docker Compose atau langsung di host WSL.
2. Database PostgreSQL target berjalan pada host WSL (dapat diakses via `host.docker.internal` dari dalam kontainer).
...
### 1. Koneksi Database PostgreSQL (Host WSL)
```env
DB_CONNECTION=pgsql
...
```bash
docker compose exec gdrive-service php artisan db:backup
```
...
```bash
docker compose exec gdrive-service php artisan db:backup --async
```
...
1. **Dumping & Kompresi**: Perintah `db:backup` memicu perintah `pg_dump` dan mengompres hasilnya langsung menggunakan `gzip` (`.sql.gz`) demi efisiensi storage.
...
Untuk memicu backup atau mengunggah berkas dari container lain di dalam docker network yang sama, Anda dapat menggunakan 2 metode berikut:
...
- **Body (Multipart Form Data)**:
  ...
  - `target_path` (string, opsional): Lokasi target di Google Drive.
...
Jika Anda memiliki akses ke server host:
